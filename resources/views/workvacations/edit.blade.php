 ...
 <div class="panel panel-primary">
    ...
    <div class="panel-body">
	  <div class="form-group @if(!empty($errors->first('date_at'))) has-error @endif">
		  	...
		  	<div class="col-sm-2">
			  	<input type="text"  class="form-control" name="date_at" id="date_at" placeholder="2017/03/01" value ="{{ old('date_at',$workVacation->date_at) }}" readOnly=true/>
		  	</div>
	  </div>
      ...
      <div class="col-sm-4">
          <select class="form-control combo_select" id="groupvacation_id" name="groupvacation_id">
            ...
            <option value=5 @if( old('groupvacation_id',$workVacation->groupvacation_id) == 5 ) selected @endif  >夏季休暇</option>
            <option value=6 @if( old('groupvacation_id',$workVacation->groupvacation_id) == 6 ) selected @endif  >冬季休暇</option>
            <option value=7 @if( old('groupvacation_id',$workVacation->groupvacation_id) == 7 ) selected @endif  >慶弔休暇</option>
            <option value=8 @if( old('groupvacation_id',$workVacation->groupvacation_id) == 8 ) selected @endif  >産休（産前休業／産後休業）</option>
            <option value=9 @if( old('groupvacation_id',$workVacation->groupvacation_id) == 9 ) selected @endif  >生理休暇</option>
            <option value=10 @if( old('groupvacation_id',$workVacation->groupvacation_id) == 10 ) selected @endif  >育児休暇</option>
            <option value=11 @if( old('groupvacation_id',$workVacation->groupvacation_id) == 11 ) selected @endif  >子の看護休暇</option>
            <option value=12 @if( old('groupvacation_id',$workVacation->groupvacation_id) == 12 ) selected @endif  >裁判員休暇</option>
            <option value=13 @if( old('groupvacation_id',$workVacation->groupvacation_id) == 13 ) selected @endif  >母性健康管理のための休暇</option>
            <option value=14 @if( old('groupvacation_id',$workVacation->groupvacation_id) == 14 ) selected @endif  >介護休業</option>
            <option value=15 @if( old('groupvacation_id',$workVacation->groupvacation_id) == 15 ) selected @endif  >介護休暇</option>
          </select>
    </div>
	  </div>
    ...
    <div id="div_change_at" class="form-group @if(!empty($errors->first('change_at'))) has-error @endif" style="display: none;">
		  	...
		  	<div class="col-sm-2">
			  	<input type="text"  class="form-control" name="change_at" id="change_at" placeholder="2017/03/01" value ="{{ old('change_at',$workVacation->change_at) }}"/>
		  	</div>
	  </div>
	  <div class="row">
	  	<div class="col-sm-12">
