...
{!! csrf_field() !!}
 <div class="panel panel-primary">
    <div class="panel-heading">
        <h3 class="panel-title">ユーザ所属編集</h3>
    </div>
    <div class="panel-body">
	  <div class="form-group">
		  	<label for="user_id" class="control-label  col-sm-12">{{affiliation->user->name}}</label>
		  	</div>
	  </div>
	  ...
	  <div class="form-group @if(!empty($errors->first('admin'))) has-error @endif" >
	          ...
	          <select class="form-control combo_select" id="admin" name="admin">
	      		...
	      		<option value="1" @if( old('admin') == "2" ) selected @endif>グループ管理者</option>
	        	<option value="99" @if( old('admin') == "99" ) selected @endif>システム管理者</option>
	          </select>
			</div>
	  </div>
